...
random_sketch=Math.floor((Math.random()*array_1.length)+1)
sketch = array_1[random_sketch]
timer="0"
score="0"
...
function draw(){
    ...
    document.getElementById("name_of_sketch").innerHTML=sketch
    strokeWeight(13);
    stroke(0);
    if( mouseIsPressed){
...
if(timer > 10000){
    timer=0
    document.getElementById("timer_value").innerHTML=timer;
    background("white");
    random_sketch=Math.floor((Math.random()*array_1.length)+1)
    sketch = array_1[random_sketch]
}
